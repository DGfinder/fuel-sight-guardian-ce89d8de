...
export default function GeraldtonLinehaulBulkEntryPage() {
  ...
  // Filter tanks for Geraldton Linehaul group
  const filteredTanks = useMemo(() => {
    if (!tanks) return [];
    
    const geraldtonLinehaulTanks = tanks.filter(t => 
      t.group_name && t.group_name.trim().toLowerCase() === 'geraldton linehaul'
    );
    
    return geraldtonLinehaulTanks;
  }, [tanks]);
            ...
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-4">
                <Button
                  variant="outline"
                  size="sm"
                  onClick={() => navigate('/geraldton-linehaul')}
                >
                  <ArrowLeft className="h-4 w-4 mr-2" />
                  Back to Dashboard
                </Button>
                <div>
                  <h1 className="text-3xl font-bold text-gray-900">Geraldton Linehaul Bulk Dip Entry</h1>
            ...
            {totalSubgroups === 0 && (
              <div className="text-center py-12">
                ...
                <p className="text-sm text-muted-foreground mt-2">
                  You may not have permission to access any Geraldton Linehaul subgroups.
                </p>
              </div>
            )}
          </div>
