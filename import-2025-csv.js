/**
 * Enhanced LYTX CSV Import Script for 2025 Data
 * Handles new date/time format and timezone parsing
 */
...
// Enhanced CSV Processing Functions for 2025 format
class Enhanced2025CSVProcessor {
  ...
  static mapRowToEvent(row) {
    ...
    // Parse combined datetime for 2025 format
  ...
  static parseDateTime(dateStr, timeStr, timezone = 'AUW') {
    try {
      // Parse date format like "5/31/25" (M/D/YY)
      ...
      // Parse time format like "7:56:46 PM"
  ...
  static transformToDbRecord(eventData) {
    const eventId = eventData.eventId || `csv_2025_${Date.now()}_${Math.random()}`;
    ...
    // Enhanced carrier detection for 2025 data
    const safeCarrier = (() => {
...
async function import2025CsvFile() {
  console.log('🚀 Enhanced LYTX CSV Import Tool - 2025 Data');
  console.log('===========================================\n');

  try {
    // Target the 2025 CSV file
    const csvPath = './Events 2025-01-01_2025-05-31.csv';
    if (!fs.existsSync(csvPath)) {
      console.log('❌ CSV file not found at:', csvPath);
      console.log('\n💡 Please ensure the 2025 CSV file is in the project directory');
      return;
    }
    ...
    console.log('📖 Reading 2025 CSV file...');
    const csvContent = fs.readFileSync(csvPath, 'utf-8');
    const { headers, rows } = Enhanced2025CSVProcessor.parseCsv(csvContent);
    ...
    for (let i = 0; i < Math.min(3, rows.length); i++) {
      const testDate = Enhanced2025CSVProcessor.parseDateTime(
        rows[i].Date, 
        rows[i].Time, 
        rows[i].Timezone
      );
    ...
    // Transform rows
    console.log('⚙️ Processing 2025 events...');
    ...
    for (let i = 0; i < rows.length; i++) {
      try {
        const mappedEvent = Enhanced2025CSVProcessor.mapRowToEvent(rows[i]);
        ...
        if (!mappedEvent.eventId) {
          mappedEvent.eventId = `csv_2025_${Date.now()}_${i}`;
        }

        const dbRecord = Enhanced2025CSVProcessor.transformToDbRecord(mappedEvent);
    ...
    const batchReference = `lytx_csv_2025_${Date.now()}`;
    const { data: batch, error: batchError } = await supabase
      .from('data_import_batches')
      .insert({
        source_type: 'lytx_events',
        source_subtype: 'CSV_2025_IMPORT',
        file_name: 'Events 2025-01-01_2025-05-31.csv',
    ...
    console.log('💾 Importing 2025 data to database...');
    const chunkSize = 100;
    let imported = 0;
    let failed = 0;
    let duplicates = 0;
    ...
    console.log(`📅 Date range: January 2025 - May 2025`);
    
    if (imported > 0) {
      console.log('\n💡 What\'s New:');
      console.log('✅ Enhanced date/time parsing for 2025 format');
      console.log('✅ Improved carrier detection logic');
      console.log('✅ Extended analytics coverage to May 2025');
      console.log('✅ Combined dataset now spans 8+ months');
      
      console.log('\n📊 Total LYTX Data Now Available:');
      console.log('• 2024 Data: ~8,898 events (Sept-Dec)');
      console.log(`• 2025 Data: ~${imported} events (Jan-May)`);
      console.log(`• Combined: ~${8898 + imported} total events`);
      
      console.log('\n🎯 Ready for Enhanced Analytics:');
      console.log('1. Visit the LYTX Safety Dashboard');
      console.log('2. Extended trend analysis now available');
      console.log('3. Year-over-year comparisons enabled');
      console.log('4. Comprehensive driver performance tracking');
    }

  } catch (error) {
    console.error('❌ 2025 import failed:', error.message);
