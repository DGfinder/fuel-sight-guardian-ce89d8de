-- ============================================================================
-- ADD HUMAN-READABLE COLUMNS TO user_group_permissions TABLE
-- ============================================================================
...
\echo 'Current user_group_permissions table structure:'
...
CREATE OR REPLACE FUNCTION populate_user_group_permissions_readable_data()
RETURNS INTEGER AS $$
DECLARE
...
BEGIN
    ...
    LEFT JOIN tank_groups tg ON user_group_permissions.group_id = tg.id
    WHERE user_group_permissions.user_id = au.id
    AND (
        user_group_permissions.user_email IS NULL OR
        user_group_permissions.user_name IS NULL OR
        user_group_permissions.group_name IS NULL
    );
...
BEGIN
    SELECT populate_user_group_permissions_readable_data() INTO updated_rows;
...
CREATE OR REPLACE FUNCTION update_user_group_permissions_readable_data()
RETURNS TRIGGER AS $$
BEGIN
...
CREATE TRIGGER trigger_update_user_group_permissions_readable
    BEFORE INSERT OR UPDATE ON user_group_permissions
    FOR EACH ROW
    EXECUTE FUNCTION update_user_group_permissions_readable_data();
...
CREATE INDEX IF NOT EXISTS idx_user_group_permissions_user_email 
ON user_group_permissions(user_email);

CREATE INDEX IF NOT EXISTS idx_user_group_permissions_group_name 
ON user_group_permissions(group_name);

\echo 'Indexes created successfully!'

\echo '=== MIGRATION COMPLETED SUCCESSFULLY ==='
\echo 'The user_group_permissions table now has human-readable columns:'
