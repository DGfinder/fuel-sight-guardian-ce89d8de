-- ============================================================================
-- FIX: Create analytics views with proper JOINs
-- Run this in Supabase SQL Editor
-- ============================================================================

-- ============================================================================
-- STEP 0: DROP existing views (required when column names change)
-- Must drop in reverse dependency order
-- ============================================================================

DROP VIEW IF EXISTS tanks_with_rolling_avg CASCADE;
DROP VIEW IF EXISTS tanks_with_latest_reading CASCADE;
DROP VIEW IF EXISTS tanks_with_latest_dip CASCADE;
DROP VIEW IF EXISTS tanks_basic_data CASCADE;
DROP VIEW IF EXISTS agbot_fleet_overview CASCADE;
DROP VIEW IF EXISTS agbot_device_health_summary CASCADE;

SELECT 'Dropped existing views' as step0_result;

-- ============================================================================
-- VIEW 1: tanks_basic_data
-- Basic tank information with current status
-- JOINs to ta_locations for address/coords and ta_products for product type
-- ============================================================================
...
SELECT 'Created tanks_basic_data view' as step1_result;
...
SELECT
  ...
  dip.measured_by as latest_dip_measured_by,
  dip.notes as latest_dip_notes
FROM tanks_basic_data t
LEFT JOIN LATERAL (
  SELECT id, level_liters, measured_at, measured_by, notes
  FROM ta_tank_dips
...
SELECT 'Created tanks_with_latest_dip view' as step2_result;
...
SELECT
  ...
  -- Latest SmartFill reading (placeholder for future)
...
SELECT 'Created tanks_with_latest_reading view' as step3_result;
...
SELECT 'Created tanks_with_rolling_avg view' as step4_result;
...
SELECT 'Created agbot_fleet_overview view' as step5_result;
...
SELECT 'Created agbot_device_health_summary view' as step6_result;

-- ============================================================================
-- VERIFICATION
-- ============================================================================

SELECT 'All 6 analytics views created successfully!' as final_result;

-- Count views created
SELECT
  (SELECT COUNT(*) FROM information_schema.views WHERE table_schema = 'public' AND table_name = 'tanks_basic_data') as tanks_basic_data,
  (SELECT COUNT(*) FROM information_schema.views WHERE table_schema = 'public' AND table_name = 'tanks_with_latest_dip') as tanks_with_latest_dip,
  (SELECT COUNT(*) FROM information_schema.views WHERE table_schema = 'public' AND table_name = 'tanks_with_latest_reading') as tanks_with_latest_reading,
  (SELECT COUNT(*) FROM information_schema.views WHERE table_schema = 'public' AND table_name = 'tanks_with_rolling_avg') as tanks_with_rolling_avg,
  (SELECT COUNT(*) FROM information_schema.views WHERE table_schema = 'public' AND table_name = 'agbot_fleet_overview') as agbot_fleet_overview,
  (SELECT COUNT(*) FROM information_schema.views WHERE table_schema = 'public' AND table_name = 'agbot_device_health_summary') as agbot_device_health_summary;

-- Test tanks_basic_data view
SELECT 'Testing tanks_basic_data:' as test;
SELECT id, name, product_type, location_name, current_level_percent
