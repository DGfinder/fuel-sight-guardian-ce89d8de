-- EMERGENCY RESTORE - Fix Broken Calculations
-- The security barrier view broke the fuel burn and days calculations
-- This script restores the exact original view definition to fix calculations
--
-- CRITICAL: This restores all calculations to working state immediately

-- ============================================================================
-- STEP 1: DROP BROKEN SECURITY BARRIER VIEW
-- ============================================================================

SELECT 'DROPPING BROKEN SECURITY BARRIER VIEW' as step;

DROP VIEW IF EXISTS tanks_with_rolling_avg;

-- ============================================================================
-- STEP 2: RESTORE EXACT ORIGINAL VIEW
-- ============================================================================

SELECT 'RESTORING ORIGINAL VIEW - EXACT COPY FROM refactor_tank_id.sql' as step;

-- This is the EXACT original definition that was working
...
-- ============================================================================
-- STEP 3: VERIFICATION THAT CALCULATIONS ARE RESTORED
-- ============================================================================

SELECT 'VERIFYING CALCULATIONS ARE RESTORED' as step;

-- Test sample calculations to ensure everything works normally
SELECT 
    'Calculation Verification' as test,
    location,
    current_level,
    rolling_avg,
    days_to_min_level,
    current_level_percent,
    'Should show normal calculated values' as expected
FROM tanks_with_rolling_avg
WHERE location LIKE '%Narrogin%'
ORDER BY location
LIMIT 5;

-- Check that view structure is back to normal
SELECT 
    'View Structure Check' as test,
    COUNT(*) as total_columns,
    'Should be same as before security barrier' as expected
...
SELECT 
    'ORIGINAL VIEW RESTORED' as status,
    'All fuel burn and days calculations should work normally again' as result,
    'Frontend filtering will be implemented separately for Sally' as next_step,
    'No more broken calculations!' as relief;